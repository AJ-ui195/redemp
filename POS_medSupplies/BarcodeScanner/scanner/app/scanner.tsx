...
export default function ScannerScreen() {
  ...
  // Simple opening animation so the user sees feedback while the scanner boots
  useEffect(() => {
  ...
  const handleAddItem = async (barcodeValue: string, barcodeType: string): Promise<void> => {
    ...
    try {
      ...
      // Send to cashier endpoint so it appears in the dashboard
      console.log('Sending request to:', API_ENDPOINTS.cashierScan);
      ...
      if (data.success) {
        Alert.alert(
          'Success',
          'Item scanned! It will appear in the cashier dashboard.',
          [
            {
              text: 'OK',
            },
  ...
  // While initializing, return a simple full-screen loader so it is always visible
  if (isInitializing) {
    return (
    ...
    <View style={styles.container}>
      ...
      <BarcodeScanner 
        ...
        onAddItem={(barcodeValue, barcodeType) => {
          console.log('📱 Scanner screen received onAddItem call:', { barcodeValue, barcodeType });
